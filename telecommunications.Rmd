...
# useful for bypassing error contrast 
...
```{r}
test = teleprices[teleprices$symbol == "T",]

row.names(test) = NULL

full_mdl = lm(close~ volume + Capital.Surplus + Gross.Margin + Liabilities 
              ,test )
...
influ = influence(full_mdl) # measure leverage points
head(sort(influ$hat, decreasing = TRUE))

# influential point if Di > 1

cooks = cooks.distance(full_mdl) # measure influential points
head(sort(cooks,decreasing = TRUE))

# decreasing leverage points
sort(influ$hat[influ$hat>(2*4)/nrow(test)],decreasing = TRUE) 
...
# doesn't change much
# mdllog = lm(log(close)~volume + Capital.Surplus + Gross.Margin + Liabilities 
#              ,test)

mdlsqrt = lm(close~volume + sqrt(Capital.Surplus) + sqrt(Gross.Margin) + Liabilities ,test)
mdllog = lm(log(close)~volume + Capital.Surplus + Gross.Margin + Liabilities 
              ,test)
mdly2 = lm(close^2~volume + Capital.Surplus + Gross.Margin + Liabilities 
              ,test)
mdlsq = lm(sqrt(close)~volume + Capital.Surplus + Gross.Margin + Liabilities 
              ,test)
mdlinv = lm(1/(close)~volume + Capital.Surplus + Gross.Margin + Liabilities 
              ,test)
